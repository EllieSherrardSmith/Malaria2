...
data.mouseTemp=read.table("C:\\Users\\Ellie\\Documents\\Data Malaria\\ALL DATA RE_ARRANGED_20062015\\3D11-50\\mouse.txt",header=T)
data.mosquTemp=read.table("C:\\Users\\Ellie\\Documents\\Data Malaria\\ALL DATA RE_ARRANGED_20062015\\3D11-50\\mosquito.txt",header=T)
...
vrs.binom<-function(p.vec){
  ...
  number[1,1:4,2,1] <- c(50,50,51,51)
  inf[1,1:4,2,1] <- s * (1-v) * c(50,50,51,51)
...
v.binom<-function(p.vec){
  ...
  # Predict q0
  number[1,1:4,1,1] <- c(95,95,95,50)
  inf[1,1:4,1,1] <- s * c(95,82,95,50)
...
vrs.binom<-function(p.vec){
  ...
  inf[1,1:4,2,2]<-5
  number[1:3,1,1,2]<-10
  number[1:3,2:3,1,2]<-c(30,10,10,30,10,10)
...
v.binom<-function(p.vec){
  ...
  v<-p.vec[1]
  r<-0.3290598 ##0.3263158 ## 0.3290598 ## ## 
  s<-0.8105263  ##0.7849003 ##
  ...
  # Predict q0
  number[1:3,1,1,1] <- c(95,95,95)
  number[1:3,2,1,1] <- c(82,95,95)
  number[1:3,3,1,1] <- c(95,100,95)
  number[1:3,4,1,1] <- c(50,50,50)
  
  inf[1,1:3,1,1] <- s * c(95,82,95)
  #inf[2,1:4,1,1] <- s * c(95,95,100,50)
  #inf[3,1:4,1,1] <- s * c(95,95,95,50)
  #inf[4,1:4,1,1] <- s * c(45,45,45,0)
  
  number[1:3,1:2,2,1] <- 50
  number[1:3,3,2,1] <- c(51,50,50)
  number[1:3,4,2,1] <- c(51,50,50)
  inf[1,1:2,2,1] <- s * (1-v) * 50
  inf[1,3:4,2,1] <- s * (1-v) * 51
  mbr.vec<-c(1,2,5,10)
  # Predcit (p1-p5 and q1-q5), loops for rounds and bites 
  for(a in 1:2){
    for(b in 1:4){
