...
conMouse<-read.table("C:\\Users\\Ellie\\Documents\\Data Malaria\\ALL DATA RE_ARRANGED_20062015\\controls\\mouse2Excluding the dead.txt",header=TRUE)
...
vrs.binom<-function(p.vec){
  ...
  # Enter p0 data
  inf[1,,1,2]<-10  ##controls Mice
  inf[1,4,1,2]<-3  ##controls Mice
  inf[1,1:3,2,2]<-5
  number[1:3,,1,2]<-10
  number[1:3,4,1,2]<-3
  ...
  data.inf<-array(NA, dim = c(4,4,2,2))    
...
v.range<-seq(-1,1,length=size.of.grid)
