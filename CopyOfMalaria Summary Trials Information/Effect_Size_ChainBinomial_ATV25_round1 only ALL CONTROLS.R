...
data.mouseTemp=read.table("C:\\Users\\Ellie\\Documents\\Data Malaria\\ALL DATA RE_ARRANGED_20062015\\ATV-25 Example\\mouse.txt",header=T)
data.mosquTemp=read.table("C:\\Users\\Ellie\\Documents\\Data Malaria\\ALL DATA RE_ARRANGED_20062015\\ATV-25 Example\\mosquito.txt",header=T)
data.mosquTemp$Treatment<-as.factor(data.mosquTemp$Treatment)
data.mouseTemp$Treatment<-as.factor(data.mouseTemp$Treatment)

data.mouseTemp<-subset(data.mouseTemp,Round==1)
data.mosquTemp<-subset(data.mosquTemp,Round==0|Round==1)
...
n.param<-3
vrs.model<-optim(rep(0.05,n.param),vrs.binom,method="L-BFGS-B",lower=rep(-0.99,n.param),upper=rep(0.99,n.param))
vrs.model
...
rEstimatesATV25<-c(ci.grid.r,na.rm=T);rEstimatesATV25 <- rEstimatesATV25[!is.na(rEstimatesATV25)];length(rEstimatesATV25)
sEstimatesATV25<-c(ci.grid.s,na.rm=T);sEstimatesATV25 <- sEstimatesATV25[!is.na(sEstimatesATV25)];length(sEstimatesATV25)


######################################################
##
## FIXED ESTIMATES FOR r AND s
##
## 
...
n.param<-1
v.model<-optim(rep(0.05,n.param),v.binom,method="L-BFGS-B",lower=rep(0,n.param),upper=rep(0.99,n.param))
v.model
...
#####################################################################################################
##                                                                                                 ##                       
### All data, all controls and all rounds - not fixed then fixed r and s                           ##  
##                                                                                                 ##  
#####################################################################################################
...
vrs.binom<-function(p.vec){
  ...
  # Enter p0 data
  inf[1,1:4,1,2]<-c(10,30,23,5)  ##controls Mice
  inf[1,1:4,2,2]<-5
  number[1:3,1,1,2]<-c(10,10,10)
  number[1:3,2:3,1,2]<-rep(c(30,10,10),2)
  number[1:3,4,1,2]<-c(23,5,5)
  number[1:3,,2,2]<-5
...
v.binom<-function(p.vec){
  ...
  v<-p.vec[1]
  r<- 0.3263158##0.3290598 
  s<- 0.8105263 ##0.7849003  
