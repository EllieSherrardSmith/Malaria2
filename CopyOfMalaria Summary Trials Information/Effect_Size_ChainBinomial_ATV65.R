...
data.mouseTemp=read.table("C:\\Users\\Ellie\\Documents\\Data Malaria\\ALL DATA RE_ARRANGED_20062015\\ATV-65\\mouse.txt",header=T)
data.mosquTemp=read.table("C:\\Users\\Ellie\\Documents\\Data Malaria\\ALL DATA RE_ARRANGED_20062015\\ATV-65\\mosquito.txt",header=T)
...
vrs.binom<-function(p.vec){
  ...
  number[1,1:4,2,1] <- c(51,51,101,101)
  inf[1,1:4,2,1] <- s * (1-v) * c(51,51,101,101)
...
rEstimatesATV65<-c(ci.grid.r,na.rm=T);rEstimatesATV65 <- rEstimatesATV65[!is.na(rEstimatesATV65)];length(rEstimatesATV65)
sEstimatesATV65<-c(ci.grid.s,na.rm=T);sEstimatesATV65 <- sEstimatesATV65[!is.na(sEstimatesATV65)];length(sEstimatesATV65)
